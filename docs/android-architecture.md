...
The app process never reads a persisted content URI after import. The importer takes a temporary URI grant and streams the source to a private staging file while hashing. It validates the book and builds chapter search units before promoting the file into private storage; the document row and search units are then committed in one Room transaction. A failed commit removes files created by that attempt. A unique SHA-256 document ID prevents copies of the same content from creating another library entry.
...
Parse, hash, search, cover decoding, and export work runs off the main thread. Library covers are decoded at thumbnail scale and kept in a bounded cache. Reading-position writes are ordered in a repository-owned scope: ordinary location changes are debounced, while leaving or pausing the reader queues the final position independently of the screen ViewModel. The reading viewport remains fixed when the temporary controls appear, avoiding an unnecessary re-pagination and location change.
